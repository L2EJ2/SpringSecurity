 ...
 *  상속 받아 응답 메시지와 페이지 경로를 설정할 수 있게 하도록 재정의를 하는 것이다.
 *  상속 후 onAuthenticationFailure() 메소드를 오버라이딩 하여 로그인 실패 시, 수행해야 할 로직을 재정의한다.
 *  이 때, 페이지 경로와 커스텀을 할 수 있도록 만들어주는 메소드는 setDefaultFailureUrl("경로") 메서드 이다.
 *  =======================================================================================================
 *  패키지 구조:
...
public class AuthFailHandler extends SimpleUrlAuthenticationFailureHandler {
    ...
    public void onAuthenticationFailure(HttpServletRequest req, HttpServletResponse resp, AuthenticationException exception)
        ...
        /* 설명. URL을 안전하게 인코딩 하는데 사용 되는 util로, 문자열을 URL에 사용 가능한 형식으로 인코딩 해준다. */
        errorMessage = URLEncoder.encode(errorMessage, "UTF-8");

        /* 설명. 로그인 실패 시, redirect할 URL을 설정 (실패 메세지를 query string으로 전달) */
