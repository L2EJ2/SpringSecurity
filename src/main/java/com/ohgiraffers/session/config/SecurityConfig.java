...
*   이를 위해 아래에서 filterChainConfigure() 메소드를 구현할 것이다.
*  */
@EnableWebSecurity
public class SecurityConfig {
    ...
    public SecurityFilterChain filterChainConfigure(HttpSecurity http) throws Exception {
        ...
        /* 설명. 접근 제어 : 서버의 리소스에 접근 가능한 권한을 URL 별로 매칭하여 설정 */
        http.authorizeHttpRequests(auth -> {
            /* 설명. 로그인, 회원가입, 실패 페이지와 Root Context는 모두에게 허용 */
            auth.requestMatchers("/auth/login", "/user/signup", "/auth/fail", "/").permitAll();
            /* 설명. "/admin/*" 엔드포인트는 "ADMIN" 권한을 가진 사용자만 접근 허용 */
            auth.requestMatchers("/admin/*").hasAnyAuthority(UserRole.ADMIN.getRole());
            /* 설명. "/user/*" 엔드포인트는 "USER" 권한을 가진 사용자만 접근 허용 */
            auth.requestMatchers("/user/*").hasAnyAuthority(UserRole.USER.getRole(), UserRole.ADMIN.getRole());
            /* 설명. 나머지 요청은 모두 인증 된(로그인한) 사용자만 접근 가능 */
            auth.anyRequest().authenticated();
        /* 설명. 로그인 관리 : <form> 태그를 사용한 로그인(form-login) 관련 설정 */
        }).formLogin(login -> {
            /* 설명. 로그인 페이지 경로 설정(로그인 페이지에 해당 되는 핸들러 매핑이 존재햐아 함 */
            login.loginPage("/auth/login");
            /* 설명. 사용자 ID 입력 필드(form 데이터 input의 name 속성과 일치) */
            login.usernameParameter("username");
            /* 설명. 사용자 PW 입력 필드(form 데이터 input의 name 속성과 일치) */
            login.passwordParameter("password");
            /* 설명. 로그인 성공 시, 이동할 기본 페이지(로그인 성공 페이지에 해당 되는 핸들러 매핑이 존재해야 함) */
            login.defaultSuccessUrl("/", true);
            /* 설명. 로그인 실패 시, 해당 예외를 처리할 핸들러 지정(직접 제작한 핸들러 사용) */
            login.failureHandler(authFailHandler);
        /* 설명. 3. 로그아웃 관리 : 로그아웃 요청 시 관련 처리 설정 */
        }).logout(logout -> {
            /* 설명. 로그아웃 경로 설정 */
            logout.logoutRequestMatcher(new AntPathRequestMatcher("/auth/logout"));
            /* 설명. 로그아웃 시, 클라이언트의 JSESSIONID 쿠키 삭제 */
            logout.deleteCookies("JSESSIONID");
            /* 설명. 세션을 무효화(소멸)하는 설정 */
            logout.invalidateHttpSession(true);
            /* 설명. 로그아웃 성공 시, 이동할 페이지의 URL 경로 */
            logout.logoutSuccessUrl("/");
        /* 설명. 4. 세션 관리 : 어플리케이션 내 세션 관리 설정 */
        }).sessionManagement(session -> {
            /* 설명. 동시 세션 수(허용 개수)를 1개로 제한 */
            session.maximumSessions(1);
            /* 설명. 세션 만료 시 이동할 페이지 URL 경로 */
